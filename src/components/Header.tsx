      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative shrink-0 hidden sm:block">
          <button
            ...
            className="w-10 h-10 rounded-full bg-white/10 border border-white/10 hover:border-emerald-500/50 flex items-center justify-center text-white font-bold shadow-sm font-mono transition-all"
            title={isAr ? "الملف التعريفي والإعدادات" : "Profile & Settings"}
          >
            {business.ownerName ? business.ownerName.charAt(0) : "K"}
          </button>
        ...
        <div>
          <h2 className="text-sm lg:text-base font-light text-white/90">
            ...
            <span className="font-semibold text-white">
              {isAr ? business.ownerNameAr || business.ownerName : business.ownerName}
            </span>
          </h2>
      ...
      <div className="flex items-center gap-2 sm:gap-3 lg:gap-5">
        ...
        <div className="text-right rtl:text-left hidden md:block">
          ...
          <p className="text-base lg:text-lg font-mono font-bold text-emerald-400">
            {netProfit > 0 ? "+" : ""}{formatCurrency(netProfit, currency, isAr)}
          </p>
        </div>
        ...
        {/* Paymob Upgrade Button */}
        <button
          onClick={() => setIsPaymobModalOpen(true)}
          className="px-3 py-1.5 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-[11px] font-bold font-mono transition-all hidden sm:flex items-center gap-1.5"
          title={isAr ? "ترقية الاشتراك عبر Paymob" : "Manage Subscription (Paymob)"}
        >
          <CreditCard className="w-3.5 h-3.5" />
          <span>{business.subscriptionTier === "business" ? "Business Plan" : isAr ? "الترقية للـ Pro" : "Upgrade Pro"}</span>
        </button>

        {/* iOS & Android Mobile Apps Trigger */}
        {onOpenFlutterArch && (
          <button
            ...
            title={isAr ? "تطبيق iOS و Android" : "iOS & Android Mobile Apps"}
          >
        ...
        <button
          ...
          className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full border flex items-center justify-center transition-all relative ${
        ...
        >
          ...
          {unreadNotificationsCount > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-emerald-500 text-black font-extrabold text-[10px] font-mono flex items-center justify-center border-2 border-[#050505] animate-pulse">
              {unreadNotificationsCount}
            </span>
          )}
        </button>
